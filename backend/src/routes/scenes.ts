...
export function createScenesRouter(db: DatabaseManager) {
  ...
  router.put('/:sceneId/objects/:id', (req, res) => {
    try {
      const { name, modelId, color, visible, locked, showNametag, transform, pathData, metadata } = req.body;
      db.updateSceneObject(req.params.id, { name, modelId, color, visible, locked, showNametag, transform, pathData, metadata });
