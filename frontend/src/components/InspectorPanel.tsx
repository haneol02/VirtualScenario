...
export default function InspectorPanel({
  ...
  useEffect(() => {
    if (selectedObject) {
      ...
      // Sync light intensity - always update to reflect latest asset metadata
      if (selectedObject.model_id?.startsWith('light_')) {
        ...
        if (asset && asset.metadata) {
          ...
          const assetIntensity = (metadata.intensity || 1.0).toString();
  ...
  }, [selectedObject, currentTime, assets]);
